...
## Who says what (STAGE deck numbering, 9 slides; slide flips match the spoken order)
...
| S4 · Solution sentence + four steps | **Vinh** | the four steps ARE the engine, his lane |
| S5 · Always-on + switch to app | **Beeds** | his box; he speaks the ten seconds and the bridge line, executes the switch himself, then goes quiet as the demo narration starts |
...
| S6 · Pays for the box | **Stephen** | business value, 30% axis |
| S7 · Receipts (0/0 · 99 min · 42 h) | **Vinh** | the person who built the evaluator states its numbers |
...
(S2 = 41% + once-a-month-by-hand for the hook; S3 = $125k + the lockout for
the agitate), and the evidence/refusal screenshots are removed because the
live app IS those slides. Arrow keys walk the exact pitch order with zero
skipping: 1 Title, 2 Problem, 3 Blind spot, 4 Solution, 5 Always-on (switch
to the app here), demo in the app, then 6 Pays-for-the-box, 7 Receipts,
8 Team+Tech, 9 Close. The 10-slide PDF stays the portal submission so
pre-eval readers get the tagline up front and the evidence and refusal
screens without a narrator.
