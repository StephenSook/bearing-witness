...
| 0.4 | vLLM serving kit NVFP4 checkpoint | `localhost:8000` | **Beeds** | ⛔ | 0.3 | marlin backend, util 0.4; KV cache is the memory hog. 2026-08-22 Stephen (from a teammate's own `TECHNICAL_REFERENCE.md` commit today, "ref: state the deployed stack"): vLLM/NVFP4 confirmed NOT in the runtime path on this box — the 0.6 parachute was taken instead. Not a status update from Beeds; flagging what the record shows so this row stops reading as untried |
| 0.5 | `qwen3_coder` parser confirmed + TTFT written down | box | **Beeds** | ⛔ | 0.4 | One-line report on Discord. Moot given 0.4 — `qwen3_coder` was the vLLM tool-call-parser flag; the Ollama/NemoClaw path actually taken doesn't use it |
| 0.6 | 11:15 parachute (only if 0.4 fails) | Ollama | **Beeds** | ✅ | — | `qwen3.6:35b-a3b-nvfp4` or `qwen3.8:27b`, native `/api/chat`. Do not feed the kit HF tarball to Ollama. 2026-08-22 Stephen (from session memory of the GB10-RUNBOOK bring-up, not re-verified live this session): `qwen3.8:27b-q4_K_M` serving via Ollama on `127.0.0.1:11434` behind NemoClaw, GPU offload confirmed (`size_vram == size`), `nemoclaw hack-agent doctor` → healthy |
| 0.7 | De-risk gates: python imports, mongod ping, model lists, OpenClaw sees model, OpenShell denies egress | box | **Beeds** + **Jadyn** | ✅ | 0.4 | All five before anyone claims the stack works. 2026-08-22 Stephen: python imports — the full pytest suite (151 tests) and bearing-witness-tools' vitest suite (33 tests) both run clean, so imports resolve. mongod ping — verified directly today (`{'ok': 1.0}`, see 2.1). OpenClaw sees model / model lists — `nemoclaw hack-agent doctor` healthy per session memory, and the six OpenClaw tools are live and callable (see 3.1). OpenShell denies egress — session memory: iptables-blocked nvidia/openrouter IPs, completed a real tool-using turn anyway; `BOX_SESSION_ENGINE.md`'s sandbox policy explicitly denies `group:fs`/`group:web`/`group:runtime`. Not independently re-run this session — citing existing evidence, not a fresh gate pass |
...
| 1.5 | **13:15 gate: one real Bearing1_3 window through engine → Mongo → UI** | **Vinh** + **Stephen** | ✅ | 1.4, 2.2 | Plain screens fine. If missed: cut localization work until detection is causal and visible. 2026-08-22 ~19:1x Stephen — ran ON THE BOX: real CLI → `engine_adapter` → gated Mongo (`bw_product/tests/test_e2e_real_engine.py`, 3/3 pass against real Bearing1_3 data: W155 red case, geometry-unverified refusal, W2 baseline honesty), plus a manual live-analysis call (window 42) landing in Mongo and read back through the UI, HUD correctly showing `MONGO LOCAL`. Gate passed |
...
| # | Component | Where | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 2.1 | MongoDB up locally, three collections + validators | box | **Stephen** | ✅ | 0.7 | `asset_configs` (immutable versioned geometry), `feature_windows` (time-series), `diagnostic_cases` (embedded task). Port bound to localhost only. 2026-08-21 18:25 Fri-night scaffold on Mac (D5 extension, disclosed); box wiring Sat. 2026-08-22 ~19:0x Stephen — box wiring DONE: no `mongod` package/binary on this box, so loaded the offline kit's docker image and tagged it `mongo:8.0-bwitness-arm64` (matches `09_VERIFY/prepare_gb10.sh`'s own retag), container `bearing-witness-mongo`, `-p 127.0.0.1:27017:27017` (LAN never exposed), data in a named docker volume (not the kit doc's `sudo mkdir` bind mount — `/opt` is root-owned on this box and sudo has no TTY here, so a plain user-owned docker volume was used instead, same local-only guarantee). `db.serverBuildInfo().modules` returns `[]`, confirming Community edition, not Enterprise. Full suite (151 tests) green against it with the real corpus mounted |
...
| 2.4 | Fleet screen (completed bearings only, exact denominator) | code | **Stephen** | ✅ | 2.3 | Judge can select only evaluated bearings. 2026-08-22 ~21:3x Stephen — STRETCH: all 15 cards now open something real, not just Bearing1_3's four curated beats. The other 14 open a live re-analysis at the exact window `run_eval.py` itself verdicted (`bw_product/live_bearing.py`, `ui/fleet_case.py`, new `/fleet/{bearing}/{condition}/{record}` route) — same engine, same gated Mongo write, deliberately a separate render path so Bearing1_3's rehearsed screens can't be touched. Threaded `cache_dir` through `engine_adapter` (latent gap: Bearing3_1 at 2538 windows took ~40s uncached, ~1s with the evaluator's own `eval/feature_cache`). Verified: 154 pytest incl. 3 new, 14-bearing Playwright sweep all pass live |
...
| 2.8 | Fixtures → real engine, everywhere | code | **Stephen** | ✅ | 1.7 | **SEAM RAN END-TO-END ON MAC Fri ~23:55**: real CLI → engine_adapter (double-validated) → gated Mongo → UI. Live W155 = ANALYST_REVIEW_REQUIRED/outer, f0 107.03125, score 30.10 (matches fixture's independent 30.1); live refusal = ABNORMAL_LOCATION_UNCONFIRMED + VERIFY_BEARING_GEOMETRY; WATCH MODE burst w1-5 all BLOCKED_BASELINE (v3 baseline honesty). LIVE ANALYSIS + START WATCH self-revealed in the UI. 2026-08-22 ~19:1x Stephen — RE-RAN ON THE BOX per this note: real-engine e2e tests pass against the real corpus + Mongo (see 1.5); watch-loop kill/restart resume verified directly against `WatchLoop`/`st.recorded_windows` (killed mid-tick after window 1 of 3, fresh instance skipped it and resumed at the next window — `skipped_on_record` confirms it); decision-gate CAS (empty reason refused, DEFER revisitable, APPROVE final, second write refused, refusal case refuses even via direct API) verified against live Mongo. Test-mutated cases dropped and UI restarted afterward to reseed clean fixtures, per `DEMO_RUNBOOK.md`'s reset procedure — box left demo-ready |
...
| # | Component | Where | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 3.1 | Six typed OpenClaw tools against the CLI/JSON | code | **Jadyn** | ✅ | 1.4 | Disable toolSearch: the model sees six tools, not a catalog. 2026-08-22 Stephen (from `docs/BOX_SESSION_ENGINE.md`, committed today): all six live in the sandbox tool policy — `diagnose_bearing, check_blockers, get_evidence, submit_decision, test_without_geometry, replay_timeline` |
| 3.2 | Confirm-before-mutate on every write | code | **Jadyn** | ✅ | 3.1 | No mutation without a human confirmation. 2026-08-22 Stephen: found + fixed the reason it wasn't firing — `confirmGate.js` was a second `package.json` extensions entry, and OpenClaw's loader only imports the first for a path-installed plugin, so the policy registered in source but never at runtime (proven by an instrumented trace that never fired). Fix: `confirmGate.ts` now exports `registerConfirmGate()`, called directly from `index.ts`'s one loaded entry. 33/33 vitest green, TS build clean, pushed. Still open: an actual human-in-the-loop click test via the sandbox dashboard (`docs/BOX_SESSION_ENGINE.md`'s own next step) — this fixes the wiring, it doesn't replace that live check |
| 3.3 | OpenShell deny-by-default policy | policy | **Jadyn** | ✅ | 0.7 | 403 on raw files, dashboard, all egress. The denied request is a demo beat. 2026-08-22 Stephen (from `BOX_SESSION_ENGINE.md` + session memory): sandbox tool policy explicitly denies `group:fs`/`group:web`/`group:runtime`, sourced `"global"`; egress hardening independently proven via iptables-blocking nvidia/openrouter IPs and completing a real tool-using turn anyway |
| 3.4 | Evidence locators on every model claim | code | **Jadyn** | ✅ | 3.1 | `asset|window|sha8|view|freq|harmonic[|sideband]` format from the contract. 2026-08-22 Stephen: seen firsthand in today's UI recording, exact format — `XJTU-SY/35Hz12kN/Bearing1_3\|w155\|b5b90d9f\|envelope\|107.03Hz\|h1` (`docs/demo/backup_demo_walkthrough.webm`) |
| 3.5 | LLM draft post-scan | code | **Jadyn** | ✅ | 3.1 | Ban `replace` always; ban `confirmed` unless status is `ANALYST_REVIEW_REQUIRED` or `INSPECTION_APPROVED`; whole words; fail = swap safe JSON line; NO retry. 2026-08-22 ~21:5x Stephen — `postScan()` was written+unit-tested but never called (same bug shape as 3.2); wired via new `sessionScan.ts` on OpenClaw's `after_tool_call`/`reply_payload_sending` hooks (sessionKey-correlated, no retry, matches spec exactly). 8 new unit tests, deployed to the live sandbox (hit + fixed an unrelated plugin-ownership block along the way). Live-fire proof inconclusive by design — the model declined the adversarial prompt on its own rather than emitting the banned text, so the hook itself wasn't exercised through that path; confidence rests on unit tests + a clean non-crashing live deploy, not a live catch. Full writeup in `docs/BOX_SESSION_ENGINE.md` |
...
| 4.8 | Pitch script rehearsed x3 timed + backup video recorded | **Stephen** | 🟡 | 4.2 | v2 DRAFTED Fri night (`docs/PITCH_SCRIPT.md`), full Sookra Pitch Arc: sourced-stat hook (EPRI 41% / ABB $125k named), agitate, stack lands in step 06 not 02, Q&A prep incl. three-move rule, demo-driver role. RECORD one full rehearsal as the hardwired backup video; if the live demo dies, cut to it without hesitation. Refusal beat = the differentiator; pick-any-window flex ONLY if 2.8 green x3. 2026-08-22 ~19:4x Stephen — backup video HALF done: `docs/demo/backup_demo_walkthrough.webm` committed (fleet → W011 → W060 → W155 red/evidence/APPROVE → W155\* refusal), recorded on the box against real Mongo, `BW_FX=off`, 16 inline DOM assertions passed + a full-decode integrity scan, script alongside it for reruns. This is a SILENT screen capture, not the narrated rehearsal take this row also calls for — the x3 timed spoken rehearsal + a human-narrated version are still outstanding |
